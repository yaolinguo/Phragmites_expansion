...
data_phragmites_zr <- data_phragmites_zr %>%
  mutate(
    Basin_clean = str_replace_all(Basin, "-", " "),
    Status = case_when(
      Basin_clean %in% c("Calcasieu Sabine","Mermentau","Teche Vermilion","Terrebonne") ~ "Expanding",
      Basin_clean %in% c("Atchafalaya","Barataria","Breton Sound","Pontchartrain")      ~ "Stable",
      Basin_clean == "Mississippi River Delta"                                          ~ "Declining",
      TRUE ~ NA_character_
    )
  ) %>%
  dplyr::select(-Basin_clean)

table(data_phragmites_zr$Status, useNA = "ifany")
