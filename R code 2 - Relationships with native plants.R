##############################################
### Guo et al. 2025 - Phragmites expansion ###
##############################################

### R code 2 - Testing the impact of Phragmites expansion on native plants
library(glmmTMB)
library(cowplot)
library(nortest)
library(sf)
...
data_Spartina_patens_zr  <- data_Spartina_patens %>%
  ...
  group_modify(~ {
    ...
    .x <- .x %>%
      mutate(pr.Spartina_patens = pr_value, zr.Spartina_patens = zr_value, var.Spartina_patenss = var_value, n.Spartina_patens = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Vigna_luteola_zr  <- data_Vigna_luteola %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Vigna_luteola = pr_value, zr.Vigna_luteola = zr_value, var.Vigna_luteolas = var_value, n.Vigna_luteola = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Ipomoea_sagittata_zr  <- data_Ipomoea_sagittata %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Ipomoea_sagittata = pr_value, zr.Ipomoea_sagittata = zr_value, var.Ipomoea_sagittatas = var_value, n.Ipomoea_sagittata = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Bolboschoenus_robustus_zr  <- data_Bolboschoenus_robustus %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Bolboschoenus_robustus = pr_value, zr.Bolboschoenus_robustus = zr_value, var.Bolboschoenus_robustuss = var_value, n.Bolboschoenus_robustus = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Sagittaria_lancifolia_zr  <- data_Sagittaria_lancifolia %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Sagittaria_lancifolia = pr_value, zr.Sagittaria_lancifolia = zr_value, var.Sagittaria_lancifolias = var_value, n.Sagittaria_lancifolia = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Polygonum_punctatum_zr  <- data_Polygonum_punctatum %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Polygonum_punctatum = pr_value, zr.Polygonum_punctatum = zr_value, var.Polygonum_punctatums = var_value, n.Polygonum_punctatum = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Schoenoplectus_americanus_zr  <- data_Schoenoplectus_americanus %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Schoenoplectus_americanus = pr_value, zr.Schoenoplectus_americanus = zr_value, var.Schoenoplectus_americanuss = var_value, n.Schoenoplectus_americanus = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Leersia_hexandra_zr  <- data_Leersia_hexandra %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Leersia_hexandra = pr_value, zr.Leersia_hexandra = zr_value, var.Leersia_hexandras = var_value, n.Leersia_hexandra = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Distichlis_spicata_zr  <- data_Distichlis_spicata %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Distichlis_spicata = pr_value, zr.Distichlis_spicata = zr_value, var.Distichlis_spicatas = var_value, n.Distichlis_spicata = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Spartina_alterniflora_zr  <- data_Spartina_alterniflora %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Spartina_alterniflora = pr_value, zr.Spartina_alterniflora = zr_value, var.Spartina_alternifloras = var_value, n.Spartina_alterniflora = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Sagittaria_platyphylla_zr  <- data_Sagittaria_platyphylla %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Sagittaria_platyphylla = pr_value, zr.Sagittaria_platyphylla = zr_value, var.Sagittaria_platyphyllas = var_value, n.Sagittaria_platyphylla = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Typha_latifolia_zr  <- data_Typha_latifolia %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Typha_latifolia = pr_value, zr.Typha_latifolia = zr_value, var.Typha_latifolias = var_value, n.Typha_latifolia = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Panicum_hemitomon_zr  <- data_Panicum_hemitomon %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Panicum_hemitomon = pr_value, zr.Panicum_hemitomon = zr_value, var.Panicum_hemitomons = var_value, n.Panicum_hemitomon = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Amaranthus_australis_zr  <- data_Amaranthus_australis %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Amaranthus_australis = pr_value, zr.Amaranthus_australis = zr_value, var.Amaranthus_australiss = var_value, n.Amaranthus_australis = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Ludwigia_grandiflora_zr  <- data_Ludwigia_grandiflora %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Ludwigia_grandiflora = pr_value, zr.Ludwigia_grandiflora = zr_value, var.Ludwigia_grandifloras = var_value, n.Ludwigia_grandiflora = n_value)
    return(.x)
  }) %>%
  ungroup()
...
data_Mikania_scandens_zr  <- data_Mikania_scandens %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Mikania_scandens = pr_value, zr.Mikania_scandens = zr_value, var.Mikania_scandenss = var_value, n.Mikania_scandens = n_value)
    return(.x)
  }) %>%
  ungroup()
...
summary(model)
model <- lm(zr.Mikania_scandensa ~ zr.phragmites, data = subset(data_merged_1, Status == "Stable"))
summary(model)
...
data_Cicuta_maculata_zr  <- data_Cicuta_maculata %>%
  ...
  group_modify(~ {
    ...
    var_value <- ifelse(n_value > 3, 1 / (n_value - 3), NA)
    .x <- .x %>%
      mutate(pr.Cicuta_maculata = pr_value, zr.Cicuta_maculata = zr_value, var.Cicuta_maculatas = var_value, n.Cicuta_maculata = n_value)
    return(.x)
  }) %>%
  ungroup()
...
### Figure 2 - Sagittaria lancifolia
data_merged_1 <- inner_join(data_phragmites_zr, data_Sagittaria_lancifolia_zr, by = "Station_ID")
model <- lm(zr.Sagittaria_lancifolia ~ zr.phragmites,  data = data_merged_1)
summary(model)
model <- lm(zr.Sagittaria_lancifolia ~ zr.phragmites, data = subset(data_merged_1, Status == "Expanding"))
...
### Figure 2 - Sagittaria platyphylla
data_merged_1 <- inner_join(data_phragmites_zr, data_Sagittaria_platyphylla_zr, by = "Station_ID")
model <- lm(zr.Sagittaria_platyphylla ~ zr.phragmites,  data = data_merged_1)
summary(model)
model <- lm(zr.Sagittaria_platyphylla ~ zr.phragmites, data = subset(data_merged_1, Status == "Expanding"))
