...
### R code 1 - Data cleaning and expansion rate
library(glmmTMB)
library(cowplot)
library(nortest)
library(sf)
...
site_ids_data <- unique(data1$Site_ID)
length(site_ids_data)
site_ids_data1 <- unique(data1$Site_ID)
site_ids_only_in_data <- setdiff(site_ids_data, site_ids_data1)
site_ids_only_in_data1 <- setdiff(site_ids_data1, site_ids_data)
site_ids_only_in_data1 <- setdiff(data1$Site_ID, data$Site_ID)
cat("Site_IDs only in data1 but not in data:\n")
print(site_ids_only_in_data1)
...
data <- data %>% 
  filter(!(is.na(Community) | Community == "" | is.na(In_out) | In_out == "")) %>%
  mutate(Cover = if_else(In_out == "Out", 0, Cover),
         Cover = if_else(grepl("^[0-9.]+$", Cover), as.numeric(Cover), 1)) %>%
...
data_phragmites_beta <- data_phragmites %>%
  mutate(Cover_prop = (Cover + 0.5) / 101) 
model_zibeta <- glmmTMB(Cover_prop ~ Collection_year * Basin + (1|Basin:Site_ID),
...
data_sf <- st_as_sf(data_phragmites, coords = c("Longitude", "Latitude"), crs = 4326)
...
p <- ggplot() +
  ...
  scale_fill_gradient2(low = "#0000CD", mid = "#FFFFFF", high = "#8B0000", 
                       midpoint = 0, limits = c(0, 100), name = "Phragmites zr") +
...
# Figure 3 - Phragmites expansion rate
...
dat2
dat <- rbind(dat1, dat2)
dat$Basin <- as.character(dat$Basin)
...
unique_count <- length(unique(data_phragmites_zr$zr.phragmites))
print(unique_count)
