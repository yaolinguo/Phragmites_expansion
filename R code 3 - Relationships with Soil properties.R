...
### R code 3 - the relationship between zr and soil traits
### Soil data cleaning
...
# Belowground_live_biomass
data_soil_Belowground_live_biomass <- data_soil[, c("Site_ID",
                                                    "Station_ID", 
                                                    "Collection_year", 
                                                    "Sample_depth",
                                                    "Belowground_live_biomass")]

data_soil_Belowground_live_biomass <- data_soil_Belowground_live_biomass %>% filter(Belowground_live_biomass != 0 & !is.na(Belowground_live_biomass))
depth_levels <- c("0 to 4", "4 to 8", "8 to 12", "12 to 16", "16 to 20", "20 to 24")
data_soil_Belowground_live_biomass <- data_soil_Belowground_live_biomass %>% filter(Sample_depth %in% depth_levels)
data_soil_Belowground_live_biomass_station <- data_soil_Belowground_live_biomass %>%
  group_by(Station_ID, Collection_year) %>%
  mutate(Belowground_live_biomass_station = mean(Belowground_live_biomass, na.rm = TRUE)) %>%
  slice(1) %>%
  ungroup()
data_soil_Belowground_live_biomass_site <- data_soil_Belowground_live_biomass_station %>%
  group_by(Site_ID, Collection_year) %>%
  mutate(Belowground_live_biomass_site = mean(Belowground_live_biomass_station, na.rm = TRUE)) %>%
  slice(1) %>%
  ungroup()
...
data_soil_Belowground_live_biomass_site$Collection_year <- as.character(data_soil_Belowground_live_biomass_site$Collection_year)
data_soil_Belowground_live_biomass_site <- filter(data_soil_Belowground_live_biomass_site, Belowground_live_biomass_site != 0)

# Belowground_dead_biomass
data_soil_Belowground_dead_biomass <- data_soil[, c("Site_ID",
                                                    "Station_ID", 
                                                    "Collection_year", 
                                                    "Sample_depth",
                                                    "Belowground_dead_biomass")]

data_soil_Belowground_dead_biomass <- data_soil_Belowground_dead_biomass %>% filter(Belowground_dead_biomass != 0 & !is.na(Belowground_dead_biomass))
depth_levels <- c("0 to 4", "4 to 8", "8 to 12", "12 to 16", "16 to 20", "20 to 24")
data_soil_Belowground_dead_biomass <- data_soil_Belowground_dead_biomass %>% filter(Sample_depth %in% depth_levels)
data_soil_Belowground_dead_biomass_station <- data_soil_Belowground_dead_biomass %>%
  group_by(Station_ID, Collection_year) %>%
  mutate(Belowground_dead_biomass_station = mean(Belowground_dead_biomass, na.rm = TRUE)) %>%
  slice(1) %>%
  ungroup()
data_soil_Belowground_dead_biomass_site <- data_soil_Belowground_dead_biomass_station %>%
  group_by(Site_ID, Collection_year) %>%
  mutate(Belowground_dead_biomass_site = mean(Belowground_dead_biomass_station, na.rm = TRUE)) %>%
  slice(1) %>%
  ungroup()
data_soil_Belowground_dead_biomass_site$Collection_year <- as.character(data_soil_Belowground_dead_biomass_site$Collection_year)
data_soil_Belowground_dead_biomass_site <- filter(data_soil_Belowground_dead_biomass_site, Belowground_dead_biomass_site != 0)
...
# Figure 5 - Soil_Moisture_Content
data_merged_model <- inner_join(data_phragmites_zr_site, data_soil_Soil_Moisture_Content_site, by = c("Site_ID"))
